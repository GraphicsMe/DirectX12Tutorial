...
private:
		...
		for (int i = 0; i < VET_Max; ++i)
		{
			...
			if (m_mesh_data->HasVertexElement(elmType))
			{
				UpdateBufferResource(commandList, &m_vertexBuffer[i], &m_intermediateVertexBuffer[i], 
					m_mesh_data->GetVertexCount(), m_mesh_data->GetVertexStride(elmType), m_mesh_data->GetVertexData(elmType));
				m_vertexBufferView[i].BufferLocation = m_vertexBuffer[i]->GetGPUVirtualAddress();
				m_vertexBufferView[i].StrideInBytes = m_mesh_data->GetVertexStride(elmType);
	...
	void SetupIndexBuffer(ComPtr<ID3D12GraphicsCommandList> commandList)
	{
		UpdateBufferResource(commandList,
			...
			m_mesh_data->GetIndexCount(), sizeof(uint32_t), (const void*)m_mesh_data->GetIndexData());
		...
		m_indexBufferView.SizeInBytes = m_mesh_data->GetIndexSize();
	}
