...
private:
	...
	void SetupVertexBuffer(ComPtr<ID3D12GraphicsCommandList> commandList)
	{
		...
		*/
		...
		UpdateBufferResource(commandList, &m_vertexPositionBuffer, &m_intermediateVertexBuffer[0], _countof(vertexPosData), sizeof(VertexPos), vertexPosData);
		...
		m_vertexPositionBufferView.StrideInBytes = sizeof(VertexPos);
		m_vertexPositionBufferView.SizeInBytes = sizeof(vertexPosData);

		UpdateBufferResource(commandList, &m_vertexColorBuffer, &m_intermediateVertexBuffer[1], _countof(vertexColData), sizeof(VertexCol), vertexColData);
		...
		m_vertexColorBufferView.StrideInBytes = sizeof(VertexCol);
		m_vertexColorBufferView.SizeInBytes = sizeof(vertexColData);

		UpdateBufferResource(commandList, &m_vertexTexBuffer, &m_intermediateVertexBuffer[2], _countof(vertexTexData), sizeof(VertexTex), vertexTexData);
	...
	void FillCommandLists(ComPtr<ID3D12GraphicsCommandList> commandList)
	{
		...
		commandList->IASetVertexBuffers(0, 1, &m_vertexPositionBufferView);
		commandList->IASetVertexBuffers(1, 1, &m_vertexColorBufferView);
		commandList->IASetVertexBuffers(2, 1, &m_vertexTexBufferView);
	...
	void SetupTexture(const std::wstring& FileName)
	{
		...
		FCommandQueue* commandQueue = D3D12RHI::Get().GetCommandQueue(D3D12_COMMAND_LIST_TYPE_DIRECT);
		ComPtr<ID3D12GraphicsCommandList> commandList = commandQueue->GetCommandList();
...
private:
	...
	ComPtr<ID3D12Resource> m_intermediateVertexBuffer[3];
